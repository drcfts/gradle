...
When using `@OutputFiles` or `@OutputDirectories` with an `Iterable<File>` type, Gradle used to disable caching for the task with the following message:
...
This is no longer the case, and using such properties doesn't prevent th4 task from being cached anymore. 
